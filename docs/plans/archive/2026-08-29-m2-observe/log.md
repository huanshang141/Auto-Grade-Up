...
## 2026-08-29 · Task 6.1
- Tried:  每个任务的 Verify 逐一重跑——1.1（pip install + 全量）、2.1 与
          3.1~3.3（四文件 91 passed）、4.1（重跑处理脚本 + 18 张尺寸断言）、
          4.2（15 节点断言）、4.3（重录 18 份转储 + 形状断言）、5.1（e2e
          18 passed）、5.2（全量 318 passed + README 在位）；处理脚本与
          录制脚本重跑后 git status 均无变化（幂等复验）
- Result: M2 验收两条逐条对照——(1) 截图样本回放：18 张夹具经
          test_observe_e2e.py 离线回放，与运行时同引擎、同模型、同区域
          定义，17 张全字段断言、fig5 按可见范围放宽并在用例内注明；
          (2) 字段解析断言全部通过：全量 318 passed。缺口如实呈现五条
          （详见 test/fixtures/incoming/README.md）
- Now:    缺口清单修正一条并重排——原 G5「物理伤害加成、元素伤害加成作
          副词条无样本」经核对游戏机制撤销：原神副词条池只含固定值与
          百分比的生命值、攻击力、防御力、元素精通、元素充能效率、暴击率、
          暴击伤害十族，伤害加成两族只能作空之杯主词条（L4 已覆盖火伤杯），
          现有夹具的副词条十族已全部有样本；原 G6（1~3 星）重编为 G5。
          随验收给出建议补拍清单八张（L9~L14、E4~E5），写入
          incoming/README.md，与缺口编号一一对应
- Convention: 观测契约折叠为正本 docs/specs/observation.md（删「落地后
          折叠」自指行，上游约束补归档位置说明）；计划文件夹按惯例移入
          docs/plans/archive/（log.md 随文件夹保留，永不删除）；录制脚本
          末尾「待 5.1 调优」的异常清单文案随 M2 收尾已成过时表述
          （fig5 为预期遮挡形态），留待 M3 接线时顺手更正
