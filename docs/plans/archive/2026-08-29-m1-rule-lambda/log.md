...
## 2026-08-29 · 完成动作
- 全部六任务勾选；重跑每个任务 Verify：1.1 全量 207 passed、2.1 49、
  2.2 38、3.1 62、4.1 29、5.1 14、6.1 14（均 passed）
- specs 三份契约折叠为正本：docs/specs/{rule-file-format,game-profile,judgment}.md
  （正本删去「落地后折叠至」自指行）
- 本变更文件夹移入 docs/plans/archive/；本日志随文件夹保留，永不删除
- M1 遗留到后续里程碑的事项：观测机构接线时定夺「值域校验是否拒绝重复副词条
  代号」与「全角数字/全角百分号归一化」（均见 6.1 前各条记录）
