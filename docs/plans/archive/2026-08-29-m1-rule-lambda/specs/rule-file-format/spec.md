...
- 落地后正本将折叠至 `docs/specs/rule-file-format.md`
- 本契约被后续版本静默破坏即视为回归
...
代号枚举不再固化于代码或格式定义：校验时从游戏档案读取（档案契约见 `specs/game-profile/spec.md`，依据 ADR-0004）。下表为原神档案的现行内容，作为样例与测试基准。
...
- 等价性保证：同一批合法/非法样本上，Python 校验与通用校验器结论必须一致（见任务 6.1）
