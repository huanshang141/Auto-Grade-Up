...
## 2026-09-06 · 任务 4.1 e2e 真值表扩展与全量复跑
- Tried:  真值表逐行复核 M2.5 口径：L 系列 roll_count 恒 None 显式断言、
  L1/L13/E1/E2 待激活行断言、E4 四条 1 次、E6 ①=1 与 ③=3、fig2/fig4 的
  ② 词条 2 次、E7 解锁词条 0、静态页无标记 0；置信度与警告断言维持既有线
- Result: 全量 441 项通过（基线 335 + M2.5 新增 106），无跳过项；真值表与
  27 份重录转储逐行互证
- 备注:   真值表主体在 3.1/3.2 已随实现同步（为保持每个提交全绿），本任务
  复核全部 27 行、补 L 系列显式断言并修订文件头过时计数（18 张 → 27 张）
- Now:    连跑到 4.1 完毕，停下汇报；4.2（契约折叠）等 ②④⑤ 模板素材到位后执行
