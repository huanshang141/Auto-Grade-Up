...
- 消费者：决策机构校验（权威实现 `agent/rule_lambda/schema.py`）、规则编辑器（M5，消费 JSON Schema 生成物）
- 本契约被后续版本静默破坏即视为回归
- M2.5 修订：新增可选顶层键 `roll_rule`、字段命名空间 `roll.*`、词条字段运算符白名单收紧（ADR-0006）
...
```jsonc
{
  ...
  "candidates": { /* 见下 */ },
  "rule": { /* 条件树，见下 */ },
  "roll_rule": { /* 条件树，可选；缺省 = 无次数约束，见下 */ },
  "fodder": { /* 见下 */ }
}
```

- 顶层键集合为 `version` `game` `name` `candidates` `rule` `fodder`（必填）+ `roll_rule`（可选，M2.5 起）：缺必填键或多出未知键均非法
- `candidates`（四键齐全，不可缺省）：
  - `rarity`: 数组，元素在该游戏档案 rarity_range 内（原神为 1–5）；空数组表示不限；M2.5 起元素须有该星级的单次成长上限表数据（原神档案为 4/5 星），引用无表星级非法
...
- `rule` 与 `roll_rule` 是两套独立条件树：两者都通过决策机构才输出「强化」（合并单布尔，ADR-0005）；`roll_rule` 缺省时仅按 `rule` 求值；`roll_rule` 内的叶子字段仅接受 `roll.*` 命名空间，出现其他字段非法
...
| `substat_count` | number（仅计已解锁条数） | 恒存在 |
| `remaining_rolls` | number（由等级推导） | 恒存在 |
| `main.<属性代号>` | number | 仅当主词条恰为该代号时存在，其余情况缺失；缺失语义与 `sub.*` 相同 |
| `sub.<属性代号>` | number | 可能缺失（待激活词条视为存在，求值口径为乐观可达值） |
| `roll.<属性代号>` | number（词条强化次数的可达值） | 仅 `roll_rule` 内可用；存在性同 `sub.*` |
...
- **词条字段（`sub.*` `roll.*`，M2.5 起收紧）**：仅接受 `>` `>=` `exists`，`value` 必须是数字——乐观可达值的口径只对增长型条件成立，等于/小于在可达值下语义失真（ADR-0006）；求值口径见 judgment 契约
- 其余数值字段（`level` `rarity` `substat_count` `remaining_rolls` `main.*`）：接受 `> < >= <= == !=`，`value` 必须是数字；接受 `exists`（对恒存在字段用它没有意义但合法）；`main.*` 维持当前值口径（主词条成长是确定性的）
...
- 错误必须携带节点路径，形如 `rule.all[0].any[1]` 或 `roll_rule.all[0]`，供编辑器定位到节点
- `version` 不等于 1 → 非法
...
- 生成物必须覆盖：结构（含顶层键集合与 `roll_rule` 可选性）、game 字段比对、档案代号枚举与 candidates 的 slots/rarity 元素值域、fodder.strategy 枚举（与档案 round_mechanism 同源）、运算符与字段类型兼容（`if/then`，含词条字段与 `roll_rule` 内字段的运算符白名单）、`exists` 无 `value` 的形状、值类型随字段
