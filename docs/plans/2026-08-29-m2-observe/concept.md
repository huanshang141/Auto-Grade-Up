...
禁止导入 maa 的理由：解析核心的输入输出全是纯数据，功能上不需要框架；maa 是框架原生二进制库的绑定，一旦导入，测试与运行就被钉在框架部署上，「不连真机验证」随之落空。这条禁令同时把「图像在框架、文字在自建」的分界变成失败即响的硬约束——解析中途越界调用框架（如在解析失败时重新识别）会当场报错，而不是在真机上表现为诡异行为。守卫沿用 M1 先例：冒烟测试断言导入解析核心后 maa 不在 sys.modules。
