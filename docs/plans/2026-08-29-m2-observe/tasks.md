...
- [ ] 3.2 列表页读取器
  ...
  Contract:
    ...
    confidences 逐字段取所用文字框 score 最小值
    硬失败（failures，artifact=None）: 必要区域缺失或全空 / 数值或等级解析异常 / 副词条行数 >
          substat_max / 副词条代号重复 / 部位名未收录
    警告（warnings）: 词条名未收录（原文入模）/ 副词条行数与等级一致性
    无状态：同输入两次调用结果相同
...
- [ ] 4.1 截图夹具处理与入库
  ...
  Contract:
    prepare_fixtures.py: 读 reference/pic/图1..图7 → 长边缩放到 1280×720 → 涂黑右下角 UID 区域 →
          写 test/fixtures/screenshots/fig1_list.png … fig7_enhance_gear.png；不改动 reference/；
          incoming/ 为补拍暂存目录（gitignore 其内容，保留目录）
  Verify:   python tools/prepare_fixtures.py 后：ls test/fixtures/screenshots → 7 个文件；
          python -c 尺寸断言 1280×720
  Done:     7 张入库、人工抽查无 UID；处理可重复执行（幂等）
...
- [ ] 4.3 转储录制脚本与真实转储入库
  ...
  Touch:    tools/record_ocr_dumps.py | test/fixtures/ocr_dumps/（7 份 JSON）
  Contract:
    脚本: Resource 加载 assets/resource → Tasker → 对每张夹具逐节点 post_recognition（OCR 与
          ...
          （键集合、文字框三键）；打印 maa 版本
  Verify:   python tools/record_ocr_dumps.py → 7 份转储生成且形状断言通过；ls test/fixtures/ocr_dumps → 7 个文件
...
- [ ] 5.1 离线全链路测试
  ...
  Contract:
    ...
    断言字段值（真值表写在测试内：等级、星级、部位、主词条、副词条、套装、锁定、extras）；
    fig4/fig5（弹窗遮挡）按可见范围放宽断言并在测试内注明；maa 仅经 requirements-dev 引入
  Verify:   python -m pytest test/test_observe_e2e.py -q → 全部通过
  Done:     7 张端到端断言通过；识别质量不达标处记录（replace 调优或该区域退回检测模式）
...
- [ ] 5.2 断言充实与覆盖缺口标注
  ...
  Contract:
    incoming/README.md 写明补拍清单与投放方式（放置后重跑 prepare_fixtures 与录制脚本即可扩测）；
    缺口清单（+0、3 词条、4 星、百分比主词条、def 族、强化页低等级等）写入 log.md 并逐条对应
    补拍编号
