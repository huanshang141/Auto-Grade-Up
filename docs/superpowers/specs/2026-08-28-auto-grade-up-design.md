...
| 主词条区 | 主词条名 + 数值 | OCR |
| 等级徽标 | 当前等级（如 +19） | OCR |
| 副词条列表 | 逐行「词条名+数值」（如「暴击率+3.1%」，带加号前缀）；3 词条圣遗物的第 4 条以灰色「待激活」预览行显示，两界面相同，解析排除之（见观测解析契约） | OCR |
...
| 摩拉区 | 本次需要摩拉数（右上角的摩拉余额与之相隔过远，单识别区域无法覆盖，第一版不读余额；M3 接线如需余额再补节点标定） | OCR |
...
| M2.5 | 决策机构扩展：强化剪枝 + 强化次数规则体系（词条强化次数入模、档案扩充单次强化成长上限表） | 离线验收：剪枝与次数条件的单元测试全绿；规则文件格式修订与 JSON Schema 导出同步 |
