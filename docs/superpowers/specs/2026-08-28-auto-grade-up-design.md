...
| 强化策略 | 逐回合判定：每回合（阶段放入+强化）恰好覆盖一个词条 roll 点，回读后判定继续或止损 |
...
| 狗粮来源 | 强化页「阶段放入」，素材档位下拉由用户预设；5 星素材被游戏开关默认排除；不做星级校验 |
| 候选范围 | 可配置过滤，默认仅未锁定且未满级；默认星级 [5] |
| 扫描前提 | 用户预先筛选「未锁定」+「未满级」，排序建议等级优先（排序选项清单待确认） |
...
一个 CustomRecognition（`ReadArtifact`）：单次截图，多个 ROI 并行识别，产出 Artifact 与逐字段置信度。观测发生在两个界面（游戏 UI 事实以 `reference/pic/` 截图为唯一真相来源）：
...
| 主词条区 | 主词条名 + 数值 | OCR |
| 等级徽标 | 当前等级（如 +19） | OCR |
| 副词条列表 | 逐行「词条名+数值」（无档位标记） | OCR |
| 星标行 | 星级 | 星星图标模板匹配（1~5 档） |
| 锁定图标 | 锁定状态 | 模板匹配（校验用，正常已被筛选排除） |

**强化页右侧**（回合中回读用，图2/图4/图5）：

| ROI 区域 | 内容 | 手段 |
| --- | --- | --- |
| 面包屑 | 部位 / 圣遗物名 | OCR |
| 等级与 EXP | 当前等级、级内经验进度（如 2900/35575） | OCR |
| 副词条列表 | 逐行「词条名+数值」+ 强化档位标记 ①②③ | OCR |
| 摩拉区 | 需要摩拉数、右上角余额 | OCR |
| 素材档位下拉 | 当前档位文本（仅记录进报告，不拦截） | OCR |
...
flowchart TD
    ...
    OPEN --> ROUND["回合：阶段放入 → 强化 → 等动画"]
    ROUND --> READ2["回读：等级 / EXP / 副词条"]
...
驱动方式（混合方案）：单件回合循环（阶段放入 → 强化 → 等动画 → 回读 → 判定）由 pipeline 状态机表达，享受模板匹配的等待与重试机制；批量遍历（选卡、滚动、终止）由 `agent/control.py` 的总控 CustomAction 驱动，两者以 `override_next` 粘合。

界面事实与扫描约定（依据 `reference/pic/` 截图）：

- **使用前提（v1 强制）**：用户预先在圣遗物筛选面板勾选「未锁定」+「未满级」（两者均确证存在），列表中因此不会出现锁定件与满级件；排序建议「等级优先」（若游戏提供），让升级成本低的件先被处理。**游戏排序选项清单尚未确认**，M3 实测后把使用前提写死进使用文档。
- 启动自检：批量任务要求当前处于圣遗物列表页，单件任务要求处于圣遗物详情页；界面特征不符则立即停止并提示，不做自动导航。
- 遍历方式：卡片网格不支持键盘移动选中，仅鼠标逐卡点击；自上而下、自左而右逐卡点击，读右栏预览判定；一屏扫完滚动一屏，直到列表尾。
- **回合循环不离开强化页**：强化动画结束后停在强化页。回合 = 「阶段放入 → 强化 → 等动画 → 回读」，仅止损/满级/资源不足时点返回回列表。
- **阶段放入语义**：需开启「放入设置 → 开启阶段放入功能」（图5）；放入量止步于 4/8/12/16 级节点，即**每回合恰好覆盖一个词条 roll 点**（+4/+8/+12/+16/+20；0→4 段 3 词条件在此解锁第 4 词条，同样是一个 roll 事件）。逐回合判定与词条变动天然对齐。+16→+20 段的确切行为 M3 真机确认。
- 「可快捷放入5星圣遗物作为强化素材」开关默认关闭（图5）→ 默认最多消耗到未锁定 4 星素材；素材档位下拉（1星 / 2星及以下 / 3星及以下 / 4星及以下，图4）的当前值每回合 OCR 记录进报告（仅记录，不拦截）。

狗粮策略（v1）：

- 强化页「阶段放入」放入狗粮，素材档位下拉与「放入设置」由用户在游戏内预先设置；锁定件被游戏机制天然排除，5 星素材被默认关闭的开关排除。
- 「把不想喂的圣遗物提前锁定」作为安全提示写入使用文档。
- 明确不做狗粮星级校验（2026-08-28 评审决策）；强化前校验「EXP/摩拉足够」，两者在强化页均有直接可读的 UI 依据。
- 报告如实记录每回合的等级跨度与档位标记变化。

安全红线：

- 一切消耗性操作之前，用强化页面包屑「部位 / 圣遗物名」+ 等级构成指纹（圣遗物名唯一对应套装+部位），复校与决策对象是同一件，不一致立即停止。
- 摩拉不足（强化页直接可读）→ 整体停止并写报告；素材不足以到达下一 roll 点（阶段放入后强化按钮不可用）→ 跳过该件继续下一件。
- 弃置、出售类不可逆操作不在功能范围内。
...
| 阶段放入可能消耗未锁定 4 星素材 | 误喂有价值 4 星 | 5 星由游戏开关默认保护；档位下拉由用户预设且每回合记录进报告；文档强提示锁定习惯 |
| 阶段放入在 +16→+20 段的确切行为未确认 | 回合粒度可能有偏差 | M3 真机首验，必要时按 EXP 差值自行分次放入 |
| 游戏排序选项清单未确认（等级优先是否存在） | 使用前提可能写不进文档 | M3 实测排序按钮后定稿使用文档 |
