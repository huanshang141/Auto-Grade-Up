...
| 字段命名 | 条件树内字段使用英文代号；游戏文字经字段对照文档适配为代号（收录词条名与部位名，中英对照，多语言预留），文档已定稿见 §4 |
...
flowchart TB
    ...
    TEXTMAP[("assets/resource/textmap/zh_cn.json<br/>字段对照文档")]
    GAME["原神（PC 端窗口）"]
...
**字段对照文档**：游戏界面文字与内部代号之间的对照，独立成文档维护（不内嵌在规则文件里）。它同时是观测机构的适配层——OCR 读到的游戏文字经它转换为内部代号，使规则字段与游戏语言无关（套装名按原文比较，是第一版的例外）；多语言客户端各维护一份，当前只维护中英对照。工具内部自算的字段（如 `remaining_rolls`）由代码定义，不入对照文档。已定稿内容：

- 位置与命名：`assets/resource/textmap/zh_cn.json`，随 MaaFramework 资源一起分发，按语言代码命名。
...
- 加载与校验语义：代号枚举固化在格式定义中，对照文档把文字映射到这些代号。文档缺失、语言不符或映射到不存在的代号 → 立即停止并提示（与规则文件同一纪律）；观测读到文档未收录的文字 → 该字段记为未知值并写入报告警告，该字段判定按不通过处理，不中断整个运行。
...
**字段类型系统已定案（2026-08-29）**：英文代号字段、副词条条数字段、字符串字段运算符（相等/不相等/存在，包含预留）、存在判断运算符。**字段对照文档已定稿（2026-08-29）**：位置、收录范围、格式与加载语义见上，M2 的阻塞项清除。
...
- OCR 读到的词条名、部位名经字段对照文档适配为内部代号（见 §4），套装名等其余文字按原文使用；对照文档未收录的文字记为未知值并写入报告警告，该字段判定按不通过处理，不中断运行。
...
格式定义的同源方式：决策机构 `schema.py` 提供导出命令，生成 JSON Schema 生成物到 `editor/src/generated/`，编辑器构建时消费；生成物不手工编辑——修改格式时先改 Python 侧，再重新导出。字段对照文档由编辑器直接读取资源目录文件，无副本；首次使用时设置一次路径（默认指向资源目录）。
...
controller：Win32，`window_regex: "原神|Genshin"`；截图与输入方式（PrintWindow、PostMessage 系列、Seize）按 MaaFramework 控制方式文档真机实测后选定。资源包第一版仅官服一份。发布物（README 与使用文档）需包含模拟输入类工具的账号风险免责声明。长期方向为自主实现完整图形界面并替代 MFAAvalonia（见 ADR-0002），届时本节的集成方式随 interface.json 一并替换。
...
assets/resource/textmap/zh_cn.json  # 字段对照文档（每语言一份）
...
| 里程碑 | 内容 | 验收标准 |
| --- | --- | --- |
| M1 | 数据模型 + rule_lambda + 格式校验 + 单元测试 + JSON Schema 导出命令；示例规则 | pytest 全部通过；示例规则在编辑器侧生成物与决策机构两端校验结果一致；导出命令产出合法生成物 |
| M2 | 观测机构在离线截图上验证 | 截图样本回放，字段解析断言全部通过 |
| M3 | 单件强化流程真机联调 | 真机上对一件圣遗物完成「读取 → 判定 → 强化 → 重新读取 → 止损」闭环 |
| M4 | 批量遍历 + 启动自检 + 报告 + 试运行 | 试运行扫出正确候选清单；真实运行产出完整报告 |
| M5 | 编辑器（Tauri）开发与打包 + MFAAvalonia 发布包组装 | 编辑器可用；MFA 加载 interface.json 完成一次批量任务 |
