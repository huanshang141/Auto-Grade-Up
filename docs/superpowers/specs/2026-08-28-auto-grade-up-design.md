...
叶子运算符：`> < >= <= == != exists`（exists 为存在判断，例如「有暴击率副词条」）。
...
| `sub.<词条名>` | number | 副词条数值（显示单位）；不存在时数值比较一律不通过，可用 exists 显式判断 |
| `剩余强化次数` | number | 从当前等级到 +20 还会发生的词条变动次数，等于 (20 − 等级) ÷ 4，由等级推导 |
...
版本与校验：`version` 字段标识规则文件的格式版本；编辑器与 `rule_lambda` 各自内置同一份格式定义做校验，解析或校验失败一律立即停止任务并提示（不回退到默认规则，避免用错误规则执行消耗性操作）。词条名枚举定义在 `rule_lambda` 中，编辑器内嵌同源副本，两端一致性由校验兜底。

**待定决策（M1 前专项讨论定案）**：条件树的字段类型系统——副词条条数是否作为字段、字符串字段（部位、套装）的运算符集合与「包含」运算符的取舍、字段与结构键名的用词语言（中文或英文）。2026-08-28 评审决定延后到专项讨论，定案后更新本节并同步编辑器。
...
- 条件树可视化编辑：添加、删除、嵌套并且组/或者组/叶子节点；叶子提供字段下拉（主词条、副词条枚举，如 `main.攻击力`、`sub.暴击率`、`剩余强化次数`）、运算符下拉、数值输入。
- 筛选条件与狗粮配置表单。
...
| 条件树字段类型系统未定案 | M1 无法开工 | 专项讨论定案后写入 §4 并同步编辑器，M1 前完成（2026-08-28 评审延后） |
